...
## 10) What next (first 3 execution days)

### Day 1
1. Add `packages/frontend-sdk` scaffold (`package.json`, `tsconfig.json`, `src/index.ts`).
2. Implement `getInfo()` and `postUtterance()` typed clients.
3. Add fixture-based tests for success/error payload normalization.

### Day 2
1. Implement `runAction()` and `fetchWidget()` typed clients.
2. Define a shared `LeonAPIError` shape with retry metadata.
3. Add transport timeout + abort support.

### Day 3
1. Create `apps/web-lite` skeleton (Preact + Vite).
2. Wire capability bootstrap from `/info` and render a minimal chat + quick actions shell.
3. Validate on Raspberry Pi kiosk mode and capture baseline performance numbers.

### Exit criteria for this mini-sprint
- A demo command can run web-lite locally and send one utterance end-to-end.
- API client package is reusable by iOS/watch adapters (via generated schema docs and examples).
- Measured baseline metrics are documented for future regression tracking.
